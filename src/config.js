
const API_URL = '/api/1.0';
// const API_URL = 'http://HoaxifyApp-env.eq9spv9gbn.eu-west-3.elasticbeanstalk.com/api/1.0/users';

const IMAGES_ATTACHMENTS = 'http://localhost:8080/images/attachments';
// const IMAGES_ATTACHMENTS = 'http://HoaxifyApp-env.eq9spv9gbn.eu-west-3.elasticbeanstalk.com/images/attachments/';

const IMAGES_PROFILE = 'http://localhost:8080/images/profile';
// const IMAGES_PROFILE = 'http://HoaxifyApp-env.eq9spv9gbn.eu-west-3.elasticbeanstalk.com/images/profile/';

export {
  API_URL,
  IMAGES_ATTACHMENTS,
  IMAGES_PROFILE

}
